...
export default function InquiryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidEmail(form.email)) { toast("올바른 이메일 형식이 아닙니다.", "error"); return; }
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!search.name.trim()) { toast("이름을 입력해주세요.", "error"); return; }
    if (!search.email.trim()) { toast("이메일을 입력해주세요.", "error"); return; }
    if (!isValidEmail(search.email)) { toast("올바른 이메일 형식이 아닙니다.", "error"); return; }
    setSearching(true);
    const data = await searchMyInquiries(search.name, search.email);
    setResults(data);
    setSearching(false);
    if (data.length === 0) toast("조회된 문의가 없습니다.", "info");
  };
      ...
      <div style={{ paddingTop: "var(--header-height)", minHeight: "100vh", background: "var(--surface-1)" }}>
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "32px 20px 80px" }}>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "white", borderRadius: 12, border: "1px solid var(--border)", overflow: "hidden" }}>
            <button onClick={() => setTab("write")} style={{ flex: 1, padding: "14px", fontSize: 15, fontWeight: tab === "write" ? 700 : 500, color: tab === "write" ? "var(--brand)" : "var(--text-3)", background: tab === "write" ? "var(--brand-light)" : "white", border: "none", cursor: "pointer" }}>
              문의 등록
            </button>
            <button onClick={() => setTab("check")} style={{ flex: 1, padding: "14px", fontSize: 15, fontWeight: tab === "check" ? 700 : 500, color: tab === "check" ? "var(--brand)" : "var(--text-3)", background: tab === "check" ? "var(--brand-light)" : "white", border: "none", cursor: "pointer", borderLeft: "1px solid var(--border)" }}>
              답변 확인
            </button>
          </div>
          ...
          {tab === "check" && (
            <div>
              <form onSubmit={handleSearch} style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", padding: "24px" }}>
                <p style={{ fontSize: 14, color: "var(--text-3)", marginBottom: 16 }}>문의 시 입력한 이름과 이메일로 조회합니다.</p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
                  <div>
                    <label style={labelStyle}>이름</label>
                  ...
                  <div>
                    <label style={labelStyle}>이메일</label>
                    <input style={inputStyle} type="email" placeholder="email@example.com" value={search.email} onChange={(e) => setSearch({ ...search, email: e.target.value })} />
                  </div>
                </div>
              ...
              {results !== null && results.length > 0 && (
                <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 0, background: "white", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden" }}>
                  {results.map((inq, i) => (
